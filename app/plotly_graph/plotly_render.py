...
def build_maps_from_db(
    people: List[Person],
    rels: List[Relationship],
) -> Tuple[Dict[str, List[str]], Dict[str, List[str]], Dict[str, str], Dict[str, str], Dict[str, str], List[str], Dict[str, List[str]]]:
    children_map: Dict[str, List[str]] = {}
    parents_map: Dict[str, List[str]] = {}
    gender_map: Dict[str, str] = {}
    display_name: Dict[str, str] = {}
    ...
    for p in people:
        ...
        # Handle both enum (from DB) and string (from SimpleNamespace/drafts)
        if p.sex:
            gender_map[p.id] = p.sex.value if hasattr(p.sex, 'value') else p.sex
        else:
            gender_map[p.id] = "U"
    ...
    for r in rels:
        if r.type == RelType.CHILD_OF:
            ...
            spouse_map.setdefault(r.from_person_id, []).append(r.to_person_id)
            spouse_map.setdefault(r.to_person_id, []).append(r.from_person_id)

        elif r.type == RelType.EARLIEST_ANCESTOR:
            # simplest convention: root stored as self-link (from == to)
            roots.append(r.from_person_id)

    # If no explicit roots were defined, find people who have no parents
    if not roots:
        all_person_ids = set(display_name.keys())
        children_with_parents = set(parents_map.keys())
        roots = list(all_person_ids - children_with_parents)
